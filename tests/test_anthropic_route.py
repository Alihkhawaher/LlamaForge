...
class TestAnthropicRoute(unittest.TestCase):
    def test_nonstream_happy_path(self):
        ...
        with mock.patch.object(routes, "model_state",
                               return_value={"models": [{"id": "local-m"}]}), \
             mock.patch.object(routes, "cfg", return_value={"router_host": "127.0.0.1",
                               "router_api_key": "", "anthropic_default_model": "",
                               "anthropic_shim_enabled": True, "router_port": 8080}), \
             mock.patch.object(routes, "_router_openai", return_value=(200, oai_resp)) as ro:
            status, body = routes._anthropic_messages(
    ...
    def test_router_error_becomes_anthropic_error(self):
        with mock.patch.object(routes, "model_state",
             ...
             mock.patch.object(routes, "cfg", return_value={"router_host": "127.0.0.1",
                               "router_api_key": "", "anthropic_default_model": "",
                               "anthropic_shim_enabled": True, "router_port": 8080}), \
             mock.patch.object(routes, "_router_openai", return_value=(500, {"error": "boom"})):
            status, body = routes._anthropic_messages(
                {"model": "local-m", "max_tokens": 10,
                 "messages": [{"role": "user", "content": "hi"}]}, {})
    ...
    def test_lan_key_mismatch_401(self):
        with mock.patch.object(routes, "cfg", return_value={"router_host": "0.0.0.0",
                               "router_api_key": "secret", "anthropic_default_model": "",
                               "anthropic_shim_enabled": True, "router_port": 8080}):
            self.assertFalse(routes._shim_auth_ok({"x-api-key": "wrong"}))
            self.assertTrue(routes._shim_auth_ok({"x-api-key": "secret"}))

    def test_lan_bearer_token_accepted(self):
        with mock.patch.object(routes, "cfg", return_value={"router_host": "0.0.0.0",
                               "router_api_key": "secret", "anthropic_default_model": "",
                               "anthropic_shim_enabled": True, "router_port": 8080}):
            self.assertTrue(routes._shim_auth_ok({"authorization": "Bearer secret"}))
            self.assertFalse(routes._shim_auth_ok({"authorization": "Bearer wrong"}))
    ...
    def test_localhost_auth_open(self):
        with mock.patch.object(routes, "cfg", return_value={"router_host": "127.0.0.1",
            ...
            self.assertTrue(routes._shim_auth_ok({}))
